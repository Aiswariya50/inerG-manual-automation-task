...
(async () => {
  ...
  const testCode = await run(process.execPath, [cli, 'test']);
  if (testCode !== 0) process.exit(testCode);

  // Our HTML reports are written into a per-run subfolder under `playwright-report/`.
  // Pick the newest one and open it on a free port to avoid EADDRINUSE:9323.
